...
export const quizGenerationParamsSchema = z.object({
  ...
  if (data.mcqCount + data.trueFalseCount + data.shortAnswerCount !== data.totalQuestions) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: 'mcqCount + trueFalseCount + shortAnswerCount must equal totalQuestions',
      path: ['mcqCount']
    });
  }
