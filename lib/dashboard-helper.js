...
const populateReviewData = async (reviews) => {
    const populatedReviews = await Promise.all(
        reviews.map(async (review) => {
            const student = await getUserDetails(review?.user?.id ?? review?.user?._id);
            review[
                "studentName"
...
const populateEnrollmentData = async (enrollments) => {
 ...
 const populatedEnrollments = await Promise.all(
    enrollments.map(async (enrollment) => {
        ...
        const student = await getUserDetails(enrollment?.student?.id ?? enrollment?.student?._id);
        // console.log(student);
        ...
        if (report) {
            // Calculate Progress
        const course = await getCourseDetails(enrollment?.course?.id ?? enrollment?.course?._id);
        //console.log(course);
